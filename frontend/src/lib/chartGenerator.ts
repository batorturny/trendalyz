...
// Port of backend/services/chartGenerator.js to ESM/TypeScript
// ============================================
...
export class ChartGenerator {
    ...
    constructor(windsorData: any, startDate: string | null = null, endDate: string | null = null) {
    ...
    generate(chartKey: string, params: any = {}) {
        ...
        const chartData = (this as any)[methodName](params);
        return {
            key: chartKey,
    ...
    isEmpty(data: any) {
        ...
        if (data.series?.[0]?.data?.every((v: number) => v === 0)) return true;
        return false;
    }

    generateGeneric(chartDef: any, _params?: any) {
        return { labels: [], series: [{ name: chartDef.title, data: [] }] };
    }

    // ===== HELPER GENERATORS =====

    dailyMetric(source: any[], field: string, name: string) {
    ...
    dailyMax(source: any[], field: string, name: string) {
        const grouped = this.groupByDate(source, 'date');
        const labels = Object.keys(grouped).sort();
        const data = labels.map(d => Math.max(...grouped[d].map((i: any) => parseInt(i[field]) || 0)));
        return { labels, series: [{ name, data }] };
    }

    dailyMultiMetric(source: any[], fields: [string, string][]) {
        const grouped = this.groupByDate(source, 'date');
        const labels = Object.keys(grouped).sort();
        return {
            labels,
    ...
    dailyAvg(source: any[], field: string, name: string) {
        const grouped = this.groupByDate(source, 'date');
        const labels = Object.keys(grouped).sort();
        const data = labels.map(d => {
            const items = grouped[d];
            const avg = items.reduce((s: number, i: any) => s + (parseFloat(i[field]) || 0), 0) / (items.length || 1);
            return parseFloat(avg.toFixed(2));
        });
        return { labels, series: [{ name, data }] };
    }

    dailyFloatSum(source: any[], field: string, name: string) {
        const grouped = this.groupByDate(source, 'date');
        const labels = Object.keys(grouped).sort();
        const data = labels.map(d =>
            parseFloat(grouped[d].reduce((s: number, i: any) => s + (parseFloat(i[field]) || 0), 0).toFixed(2))
        );
        return { labels, series: [{ name, data }] };
    }

    sortedTable(items: any[], sortField: string, limit: number, mapper: (v: any) => any, headers: string[]) {
        const sorted = [...items]
    ...
    generate_tt_video_watch_time() {
        const videos = this.video.filter((v: any) => v.video_average_time_watched);
        return {
            labels: videos.map((v: any) => (v.video_caption || '').substring(0, 30) || v.video_id || '-'),
            series: [{ name: 'Átl. nézési idő (mp)', data: videos.map((v: any) => parseFloat(v.video_average_time_watched) || 0) }]
        };
    }
    ...
    generate_tt_video_retention() {
        ...
        const labels = Object.keys(grouped).sort();
        const data = labels.map(date => {
            const items = grouped[date].filter((i: any) => i.video_full_watched_rate);
            if (items.length === 0) return 0;
            return parseFloat((items.reduce((s: number, i: any) => s + (parseFloat(i.video_full_watched_rate) || 0), 0) / items.length).toFixed(2));
        });
    ...
    generate_tt_gender_demographics() {
        ...
        const labels: string[] = [];
        const data: number[] = [];
        const nameMap: Record<string, string> = { 'female_vv': 'Nő', 'male_vv': 'Férfi', 'other_vv': 'Egyéb' };
    ...
    generate_engagement_by_day() {
        ...
        this.activity.forEach((item: any) => {
        ...
        return {
            labels: dayNames,
            series: [{ name: 'Átl. aktivitás', data: dayData.map((sum: number, i: number) => dayCounts[i] > 0 ? Math.round(sum / dayCounts[i]) : 0) }]
        };
    }
    ...
    generate_worst_3_videos() {
        const deduped = this._dedupeVideos(this.video).filter((v: any) => v._maxViews > 0);
        const sorted = [...deduped].sort((a, b) => a._maxViews - b._maxViews);
        return this._buildVideoTable(sorted.slice(0, 3));
    }

    _dedupeVideos(videos: any[]) {
        const map: Record<string, { rows: any[], id: string }> = {};
        videos.forEach(v => {
            const id = v.video_id;
            if (!id) return;
            if (!map[id]) map[id] = { rows: [], id };
            map[id].rows.push(v);
        });
        return Object.values(map).map(({ rows, id }) => {
            const maxInt = (field: string) => Math.max(...rows.map(r => parseInt(r[field]) || 0));
            const avgFloat = (field: string) => {
                const vals = rows.map(r => parseFloat(r[field]) || 0).filter(v => v > 0);
                return vals.length > 0 ? vals.reduce((s, v) => s + v, 0) / vals.length : 0;
            };
            const first = rows[0];
            return {
                video_id: id, video_caption: first.video_caption, video_create_datetime: first.video_create_datetime,
                video_embed_url: first.video_embed_url,
                _maxViews: maxInt('video_views_count'), _maxReach: maxInt('video_reach'),
                _maxLikes: maxInt('video_likes'), _maxComments: maxInt('video_comments'),
                _maxShares: maxInt('video_shares'), _maxNewFollowers: maxInt('video_new_followers'),
    ...
    _buildVideoTable(deduped: any[]) {
        const tableData = deduped.map(v => {
            const er = this.engagementRate(v._maxLikes, v._maxComments, v._maxShares, v._maxReach);
            return {
                id: v.video_id, caption: v.video_caption || '-',
                date: v.video_create_datetime ? v.video_create_datetime.substring(0, 10) : '-',
                views: v._maxViews, likes: v._maxLikes, comments: v._maxComments, shares: v._maxShares,
                reach: v._maxReach, newFollowers: v._maxNewFollowers,
                fullWatchRate: parseFloat((v._avgFullWatchRate * 100).toFixed(2)),
                avgWatchTime: parseFloat(v._avgWatchTime.toFixed(1)),
                engagementRate: er, link: v.video_embed_url || '#'
            };
        });
    ...
    generateFacebookPostTable(posts: any[]) {
    ...
    generate_fb_engagement_rate() {
        ...
        const data = labels.map(date => {
            ...
            return this.engagementRate(
                this.sumField(items, 'reactions'), this.sumField(items, 'comments'),
                this.sumField(items, 'shares'), this.sumField(items, 'reach') || 1
            );
        });
        return { labels, series: [{ name: 'ER %', data }] };
    }
    ...
    generate_fb_reel_performance() {
        const posts = this.video.filter((p: any) => parseInt(p.post_video_views) > 0);
    ...
    generate_ig_reel_performance() {
        ...
        const watchTime = labels.map(date => {
            const items = grouped[date];
            const avg = items.reduce((s: number, i: any) => s + (parseFloat(i.media_reel_avg_watch_time) || 0), 0) / (items.length || 1);
            return parseFloat(avg.toFixed(2));
        });
    ...
    generateInstagramMediaTable(media: any[]) {
    ...
    generate_igpub_engagement_rate() {
        ...
        const data = labels.map(date => {
            ...
            const followers = Math.max(...items.map((i: any) => parseInt(i.profile_followers_count) || 0));
    ...
    generateIGPublicMediaTable(media: any[]) {
        const tableData = media.map(m => ({
    ...
    generate_yt_avg_view_pct() {
        const videos = this.video.filter((v: any) => v.average_view_percentage);
        return {
            labels: videos.map((v: any) => (v.video_title || '').substring(0, 30) || v.video_id || '-'),
            series: [{ name: 'Nézési %', data: videos.map((v: any) => parseFloat((parseFloat(v.average_view_percentage) || 0).toFixed(2))) }]
        };
    }
    ...
    generateYouTubeVideoTable(videos: any[]) {
    ...
    generate_ttads_campaign_perf() {
        const campaignMap: Record<string, { impressions: number; clicks: number; spend: number; conversions: number }> = {};
        this.data.forEach((item: any) => {
    ...
    _aggregateByField(source: any[], keyField: string, valueField: string, seriesName: string, sortAlpha = false, limit = 0, multiplier = 1, normalize = false) {
        ...
        return {
            ...
            series: [{
                name: seriesName,
                data: sorted.map(([, v]) => {
                    if (normalize && total > 0) return parseFloat(((v / total) * 100).toFixed(2));
    ...
    groupByDate(items: any[], dateField: string) {
    ...
    sumField(items: any[], field: string) {
        if (!Array.isArray(items)) return 0;
        return items.reduce((sum, item) => sum + (parseFloat(item[field]) || 0), 0);
    }

    engagementRate(likes: number, comments: number, shares: number, divisor: number) {
        if (!divisor) return 0;
